...
function App() {
  ...
  return <main><header><button className="logo" onClick={() => setView('home')}>🌿 FairPlay</button><nav><button onClick={() => setView('home')}>게시판</button><button onClick={() => setView('profile')}>내 프로필</button></nav></header>{view !== 'profile' && <section className="hero"><div className="hero-copy"><p className="eyebrow">GROW TOGETHER</p><h1>프로젝트 결과를 공유하고<br />자유롭게 소통해 보세요</h1><p>작은 아이디어가 더 좋은 내일을 만듭니다.</p></div><div className="hero-panel"><span>오늘의 보드</span><strong>{boardPosts.length}</strong><p>공유된 프로젝트 이야기</p></div></section>}
    ...
    {view === 'profile' && <Profile user={user} posts={posts} notice={notice} onUserChange={nextUser => setUser({ ...user, ...nextUser })} openPost={post => { setSelected(post); setView('detail'); }} />}
    {message && <div className="toast">{message}</div>}
  </main>;
}
function Profile({ user, posts, notice, onUserChange, openPost }) {
  const [profile, setProfile] = useState({ id: user.id, email: '', nickname: user.name });
  const [nickname, setNickname] = useState(user.name);
  const [avatar, setAvatar] = useState(() => localStorage.getItem(`profile-avatar-${user.id}`) || '');
  const [myComments, setMyComments] = useState([]);
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [visitors] = useState(() => {
    const key = `profile-visitors-${user.id}`;
    const next = Number(localStorage.getItem(key) || 0) + 1;
    localStorage.setItem(key, String(next));
    return next;
  });
  const myPosts = posts.filter(post => Number(post.userId) === Number(user.id));
  const receivedLikes = myPosts.reduce((total, post) => total + Number(post.likes || 0), 0);

  useEffect(() => {
    api.user(user.id).then(data => { setProfile(data); setNickname(data.nickname); onUserChange({ name: data.nickname }); }).catch(e => notice(e.message));
    const realPosts = posts.filter(post => !String(post.id).startsWith('sample-'));
    Promise.all(realPosts.map(post => api.comments(post.id).catch(() => [])))
      .then(groups => setMyComments(groups.flat().filter(comment => Number(comment.userId) === Number(user.id))));
  ...
  const changeAvatar = event => {
    ...
    reader.onload = () => { const value = String(reader.result); setAvatar(value); localStorage.setItem(`profile-avatar-${user.id}`, value); notice('프로필 사진을 변경했어요.'); };
    reader.readAsDataURL(file);
  };

  return <section className="profile-page"><div className="profile-cover"><div className="profile-avatar">{avatar ? <img src={avatar} alt="프로필" /> : <span>{(profile.nickname || user.name || '?').slice(0, 1)}</span>}<label className="avatar-button">사진 변경<input type="file" accept="image/*" onChange={changeAvatar} /></label></div><div className="profile-identity"><p className="eyebrow">MY PROFILE</p>{editing ? <div className="profile-edit"><input value={nickname} onChange={e => setNickname(e.target.value)} maxLength="50" /><button onClick={() => { setEditing(false); setNickname(profile.nickname); }}>취소</button><button className="primary" onClick={saveProfile} disabled={saving}>{saving ? '저장 중...' : '저장'}</button></div> : <><h1>{profile.nickname || user.name}</h1><p>{profile.email || '회원 정보를 불러오는 중...'}</p><button className="profile-edit-button" onClick={() => setEditing(true)}>회원정보 수정</button></>}</div></div><div className="profile-stats"><div><strong>{myPosts.length}</strong><span>내가 쓴 게시물</span></div><div><strong>{receivedLikes}</strong><span>받은 좋아요</span></div><div><strong>{myComments.length}</strong><span>내가 쓴 댓글</span></div><div><strong>{visitors}</strong><span>프로필 방문</span></div></div><div className="profile-grid"><section><h2>내가 쓴 게시물</h2>{myPosts.length ? myPosts.map(post => <button className="profile-activity" key={post.id} onClick={() => openPost(post)}><strong>{post.title}</strong><span>{date(post.createdAt)}</span><p>{post.content}</p></button>) : <p className="profile-empty">아직 작성한 게시물이 없습니다.</p>}</section><section><h2>내가 쓴 댓글</h2>{myComments.length ? myComments.map(comment => <div className="profile-activity" key={comment.id}><strong>{comment.nickname || profile.nickname}</strong><span>{date(comment.createdAt)}</span><p>{comment.content}</p></div>) : <p className="profile-empty">아직 작성한 댓글이 없습니다.</p>}</section></div><p className="profile-note">프로필 사진과 방문자 수는 백엔드 API가 추가되기 전까지 이 브라우저에만 저장됩니다. 좋아요 수는 게시글 API가 likes 값을 제공하면 자동 집계됩니다.</p></section>;
}
