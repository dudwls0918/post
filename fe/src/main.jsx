...
function Login({ onLogin, error }) {
  ...
  return <main className="login"><section className="login-card"><div className="mark">🌱</div><h1>로그인</h1><p className="muted">등록된 계정으로 로그인해 주세요.</p><form onSubmit={e => { e.preventDefault(); onLogin(id, password); }}><label>아이디<input type="text" value={id} onChange={e => setId(e.target.value)} /></label><label>비밀번호<input type="password" value={password} onChange={e => setPassword(e.target.value)} /></label><button className="primary">로그인</button></form>{error && <p className="error">{error}</p>}</section></main>;
}
